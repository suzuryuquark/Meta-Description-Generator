...
class HistoryView(ft.Container):
    def __init__(self, page: ft.Page, storage: StorageService, export_service: ExportService, error_service, show_status, show_error):
        super().__init__()
        self.page = page
    ...
    def build_ui(self):
        ...
        self.content = ft.Column([
            ft.Row([
                ...
                ft.Row([
                    ft.ElevatedButton(
                        "CSVエクスポート", icon=ft.Icons.DOWNLOAD,
                        on_click=lambda e: self.page.run_task(self.export_click, e)
                    ),
                    ft.IconButton(
                        icon=ft.Icons.DELETE_SWEEP, tooltip="履歴をクリア",
                        on_click=lambda e: self.page.run_task(self.clear_history_click, e)
                    ),
                ], spacing=10),
    ...
    async def export_click(self, e):
        history = await self.storage.get_history()
        if not history: return await self.show_error("保存する履歴がありません")
        timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        self.csv_picker.save_file(file_name=f"meta_descriptions_{timestamp}.csv", allowed_extensions=["csv"])
