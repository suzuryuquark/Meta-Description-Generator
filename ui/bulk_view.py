...
class BulkView(ft.Container):
    def __init__(self, page: ft.Page, storage: StorageService, error_service, show_status, show_error, load_history_cmd, get_settings):
        super().__init__()
        self.page = page
    ...
    def build_ui(self):
        ...
        self.generate_btn = ft.ElevatedButton(
            text="一括生成を開始", icon=ft.Icons.PLAY_ARROW,
        ...
        self.content = ft.Column([
            ft.Text("複数のサイトURLからメタデータを一括生成します。", size=16, color=ft.Colors.GREY_700),
            ft.Divider(),
            self.urls_input,
            self.generate_btn,
            self.status_text,
    ...
    async def generate_bulk_click(self, e):
        ...
        if not raw_urls: return await self.show_error("URLを入力してください")
        ...
        if len(urls) > 50:
            return await self.show_error(f"一度に生成できるのは50件までです (現在{len(urls)}件)。件数を減らして再試行してください。")
        ...
        for i, url in enumerate(urls):
            try:
                ...
                suggestions = await asyncio.to_thread(
                    ...
                    api_key, text, settings.get('global_instruction'), 
                    settings.get('target_keywords'), tone=settings.get('tone')
                )
                            ...
                            content=ft.Container(
                                content=ft.Column([
                                    ft.ListTile(
                                        ...
                                        subtitle=ft.Text(f"パターン: {item['title']}", weight=ft.FontWeight.W_500)
                                    ),
                ...
                if await self.error_service.handle_exception(ex, f"一括生成 ({url})"):
                    ...
                    self.results_column.controls.append(ft.Text(f"エラー ({url}): {display_err}", color=ft.Colors.RED))
                self.update()
