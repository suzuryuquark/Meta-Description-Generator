    ...
    async def handle_exception(self, ex: Exception, context: str = ""):
        ...
        if "429" in err_msg or "quota" in err_msg.lower():
            ...
            dialog = ft.AlertDialog(
                ...
                content=ft.Text(
                    f"{context}中にAI APIの利用制限（クォータ）に達しました。しばらく時間をおいてから再試行するか、APIプランを確認してください。"
                ),
            ...
            dialog = ft.AlertDialog(
                ...
                content=ft.Text(
                    "AIサービスが一時的に混み合っているか、メンテナンス中です。数分待ってから再度お試しください。"
                ),
                actions=[ft.TextButton("閉じる", on_click=lambda _: self.page.close(dialog))],
            )
            self.page.open(dialog)
            ...
            "制限されました" in err_msg
            or "ブロックされました" in err_msg
            or "finish_reason" in err_msg
        ):
            # Safety Filter or Content Block
            dialog = ft.AlertDialog(
                title=ft.Text("AIによる生成制限"),
                content=ft.Text(
                    f"{context}中にAIのガイドラインやポリシーにより生成が制限されました。指示文の内容を見直すか、別のURLでお試しください。\n\n詳細: {err_msg}"
                ),
                actions=[ft.TextButton("閉じる", on_click=lambda _: self.page.close(dialog))],
            )
            self.page.open(dialog)
